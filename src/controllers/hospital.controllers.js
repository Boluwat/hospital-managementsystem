...
const createHospital = async (request) => {
  const {
    email, firstname, lastname, hospitalName, password, mobile,
  } = request.payload;
  ...
  const hospitalResponse = await request.server.app.services.hospitals.signupHospital({
    email,
    hospitalName,
    mobile,
...
const deleteHospital = async (request) => {
  ...
  const response = await request.server.app.services.hospitals.deactivateAccount(
    hospital,
    password,
  );
  if (response.error) {
