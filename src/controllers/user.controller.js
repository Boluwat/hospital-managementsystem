...
const { confirmAdmin, verify, confirmHospitalAdmin } = require('../utils/tokenizer');
const config = require('config');

const signInCustomUser = async (request, reply) => {
    const response = await request.server.app.services.users.signInUser(request.payload);
    if (response.error) {
...
const createUser = async (request, type) => {
    ...
    if ((type === constants.HOSPITALS && !await confirmHospitalAdmin(request))
    ...
    const response = await request.server.app.services.users.signUpUser(validateData);
    if (response.error) {
...
const resetPassword = async (request) => {
    const { user } = await verify(request.auth.credentials.token);
    const response = await request.server.app.services.users.resetPassword(request.payload, user);
    if (response.error) {
        return error(400, response.error);
    }
    return response;
...
const deactivateUser = async (request) => {
    ...
    const { user } = await verify(request.auth.credentials.token);
    const response = await request.server.app.services.users.deleteUser(password, user);
    if (response.error) {
        return error(400, response.error);
    }
    return response;
